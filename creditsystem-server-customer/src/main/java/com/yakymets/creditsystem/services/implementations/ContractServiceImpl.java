...
public class ContractServiceImpl implements ContractService {
    ...
    public ContractDTO updateContract(ContractDTO contractDTO, Long contractId) {
        ...
        contract.setOwedSum(contractDTO.getOwedSum());
        contract.setPayedSum(contractDTO.getPayedSum().add(contract.getPayedSum()));
        Contract saved = contractRepository.save(contract);
        if (isPaidOff(contract)) {
            contractRepository.delete(saved);
        }
        return new ContractDTO(saved);
    }

    private boolean isPaidOff(Contract contract) {
        return contract.getDeadline().before(new Date())
                ? contract.getOwedSum().divide(new BigDecimal(100), 3, RoundingMode.CEILING)
                .multiply(BigDecimal.valueOf(contract.getCredit().getEarningPercentageAfterDeadline()))
                .add(contract.getOwedSum())
                .subtract(contract.getPayedSum())
                .compareTo(contract.getPayedSum()) <= 0

                : contract.getOwedSum().divide(new BigDecimal(100), 3, RoundingMode.CEILING)
                .multiply(BigDecimal.valueOf(contract.getCredit().getEarningPercentage()))
                .add(contract.getOwedSum())
                .subtract(contract.getPayedSum())
                .compareTo(contract.getPayedSum()) <= 0;
    }
